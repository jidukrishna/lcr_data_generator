...
from reportlab.platypus import SimpleDocTemplate, Table, Image, TableStyle, Paragraph
...
def generate_graph(L, R, C, V, start_f, end_f, no_of_points, name_graph="graph.png"):
    def get_fr(L, C, R):
        ...
        f_resonant = (1 / (2 * np.pi * np.sqrt(L * C))) * ((1 - (R / 2 * L) ** 2) ** 0.5)
        return f_r
    ...
    def find_half_power_frequencies(frequencies, I_omega, I_half_power):
        ...
        for i in range(len(I_omega) - 1):
            if (I_omega[i] - I_half_power) * (I_omega[i + 1] - I_half_power) < 0:
                ...
                f_half = frequencies[i] + (frequencies[i + 1] - frequencies[i]) * (I_half_power - I_omega[i]) / (
                            I_omega[i + 1] - I_omega[i])
...
def generate_graph_test(L, values_of_R, C, V, start_f, end_f, no_of_points, name_graph="graph.png"):
    def get_fr(L, C, R):
        """Function to compute resonant frequency."""
        f_r = (1 / (2 * np.pi)) * np.sqrt(1 / (L * C)) * np.sqrt(1 - (R / (2 * np.sqrt(L / C))) ** 2)
        # f_resonant = (1 / (2 * np.pi * np.sqrt(L * C))) * ((1 - (R / 2 * L) ** 2) ** 0.5)
    ...
    for R in values_of_R:
        ...
        axe.plot(frequencies, I_omega,color=((R%9)*0.1,(R%7)*0.1,0.5),label=f"Current Amplitude R {R}ohm")
        ...
        def draw_lines():
            ...
            for i, f_half in enumerate(half_power_frequencies):
                print(i, f_half)
                b.append(f_half)
                axe.plot([f_half,f_half],[0,i_707] ,color=k[c], linestyle='--', label=f"{g[c]} Frequency {i + 1}: {f_half:.2f} Hz")
                c += 1
            # Mark the resonant frequency and add it to the legend
            imax_cord = []
            ...
            # Draw horizontal line at I_max * 0.707 (half-power point) and add it to the legend
            axe.plot(b,[i_707,i_707], color='purple', linestyle='--',
                        label=f"Imax*0.707: {round(i_707, 2)} mA")

        def scatter_plots():
            axe.scatter(f_resonant, I_max, color='r')
            k = ("g", "black")
            c = 0
            for i, f_half in enumerate(half_power_frequencies):
                axe.scatter(f_half, i_707, color=k[c])
                c += 1
...
def generate_pdf(L, R, C, V, start_f, end_f, no_of_points, pdf_file=f"electrical damping lcr.pdf",
                 image_name_graph="graph.png"):
    ...
    main_heading = Paragraph(f"electrical damping for an LCR circuit R= {R}", styles["Heading1"])
...
def make_pdf_all(L, R, C, V, start_f, end_f, no_of_points, pdf_file, name_of_graph_pic):
    R=[float(i.strip()) for i in str(R).split(" ")]
    if len(R):
        names = PdfWriter()
        temp_files = []
        for i in R:
            generate_pdf(L, i, C, V, start_f, end_f, no_of_points, f"temp_pdf_R_=_{i}.pdf", f"temp graph R {i}.png")
            names.append(f"temp_pdf_R_=_{i}.pdf")
            temp_files.append(f"temp_pdf_R_=_{i}.pdf")
            temp_files.append(f"temp graph R {i}.png")

        generate_graph_test(L, R, C, V, start_f, end_f, no_of_points, name_of_graph_pic)
        image=pilimage.open(name_of_graph_pic)
        with open(f"temp_all_graph.pdf", "wb") as f:
            f.write(img2pdf.convert(image.filename))
        names.append(f"temp_all_graph.pdf")
        temp_files.append(f"temp_all_graph.pdf")
        names.write(pdf_file)
        names.close()
        for i in temp_files:
            os.remove(i)
    else:
        generate_pdf(L, R[0], C, V, start_f, end_f, no_of_points, pdf_file, name_of_graph_pic)
...
def main():
    ...
    c.CTkLabel(root, text="2. for multiplication use this format 3*10**2\nwhich is same as 3x10^2 except for resistors").grid(column=0,
                                                                                                         row=12,
                                                                                                         rowspan=2)
    c.CTkLabel(root, text="3.for damping simulation of multiple R values use '  '(space)\n to "
                          "separate it eg : 47 100 150").grid(column=0, row=14)

    c.CTkLabel(root, text="4. use .pdf and .png while saving").grid(column=0, row=15)
    c.CTkLabel(root, text="made by jidu krishna , cs-c 28\'").grid(column=1, row=16)
    ...
    def make_it():
        ...
        make_pdf_all(L, R, C, V, start_f, end_f, no_of_points, pdf_file, name_of_graph_pic)
...
def test():
    ...
    C = 0.1 * 10 ** -6  # capacitance in Farads
    V = 5.0  # voltage source amplitude in Volts
    start_f = 100  # starting frequency in Hz
    end_f = 7000  # ending frequency in Hz
    no_of_points = 100  # number of points for frequency sweep
    name_of_graph_pic = "47 100 150 graph.png"
    pdf_file = f"47 100 150.pdf"
    R=[float(i.strip()) for i in str(R).split(",")]
    if len(R):
        names = PdfWriter()
        temp_files = []
